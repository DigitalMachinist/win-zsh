...
- I'm annoyed by how my Windows Terminal `Settings.json` copy/paste bindings collide with \*nix commands and would love to have better suggestions. For example, `Ctrl+C` sends `SIGINT` in \*nix and I don't want to kill commands that are running to copy text since apparently both happen at the same time. Also, in vim `Ctrl+V` enters visual mode but Windows Terminal's `Ctrl+V` bind seems to block vim from doing this.
